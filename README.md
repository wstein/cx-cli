...
| Command | Description |
|---------|-------------|
| `cx bundle [path] [--zip [name]]` | Scan folder (default current directory), write `manifest.json` + `SHA256SUMS`, optional zip; `--sections` generates repomix outputs from `cx.json` before bundling |
